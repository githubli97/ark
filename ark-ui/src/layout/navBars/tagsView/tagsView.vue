...
export default {
	...
	setup() {
		...
		const getTagsViewRoutes = async () => {
			...
			state.tagsViewList = [];
			state.tagsViewRoutesList = store.state.router.tagsViewRoutes;
			initTagsView();
		};
		...
		const openCurrenFullscreen = async (path: string) => {
			...
			store.dispatch('router/setCurrenFullscreen', true);
		};
		...
		watch(store.state, (val) => {
			if (val.router.tagsViewRoutes.length === state.tagsViewRoutesList.length) return false;
			getTagsViewRoutes();
		});
		return {
			isActive,
