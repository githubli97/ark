	...
	setup() {
		...
		const setFilterRoutes = () => {
			...
			if (layout === 'classic' && isClassicSplitMenu) {
				state.menuList = delClassicChildren(filterRoutesFun(store.state.router.routesList));
				const resData = setSendClassicChildren(route.path);
				proxy.mittBus.emit('setSendClassicChildren', resData);
			} else {
				state.menuList = filterRoutesFun(store.state.router.routesList);
			}
		};
		...
		const setSendClassicChildren = (path: string) => {
			...
			let currentData: any = {};
			filterRoutesFun(store.state.router.routesList).map((v, k) => {
